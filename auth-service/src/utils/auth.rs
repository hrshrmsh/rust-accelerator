...
#[instrument(name = "Generating auth cookie", skip_all)]
...
fn generate_auth_token(email: &Email) -> Result<String, GenerateTokenError> {
    let delta = Duration::try_seconds(TOKEN_TTL_SECONDS).ok_or_else(|| {
        GenerateTokenError::UnexpectedError(eyre!("could not create token duration"))
    })?;

    let expiration: usize = Utc::now()
        .checked_add_signed(delta)
        .ok_or_else(|| {
            GenerateTokenError::UnexpectedError(eyre!("could not generate token expiry"))
        })?
        .timestamp()
        .try_into()
        .map_err(|e: TryFromIntError| GenerateTokenError::UnexpectedError(e.into()))?;
...
#[instrument(name = "Validating auth token", skip_all)]
pub async fn validate_token(
    ...
    if banned_token_store
        .check_token(token)
        .await
        .map_err(|e| GenerateTokenError::UnexpectedError(e.into()))?
    {
